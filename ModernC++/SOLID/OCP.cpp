...
{
	virtual std::vector<Product*>Find(std::vector<Product*> items, ISpecification<Product>& spec) override
	{
		Items results;
		for (auto& p : items)
...
int main(int argc, char ** argv)
{
	...
	for(auto& i : results)
	{
		std::cout << i->m_Name << " : is " << vColor[(int)i->m_Color].c_str() << " and " << vSize[static_cast<int>(i->m_Size)].c_str() << std::endl;
	}
